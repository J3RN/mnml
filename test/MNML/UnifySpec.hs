...
import           MNML.AST.Type        (Typed (..), ValueDef)
...
expectBatch :: Either [Error] TAST.Batch -> (TAST.Batch -> Expectation) -> Expectation
expectBatch (Left errs) _        = unexpected errs
expectBatch (Right batch) expect = expect batch

expectValue :: Either [Error] TAST.Batch -> QualifiedValueReference -> (TAST.ValueDef -> Expectation) -> Expectation
...
spec =
    describe "literals" $ do
      it "unifies 'nUmeRiCs'" $ do
        expectValue
          ...
          ([], "main")
          (\case
             (TAST.ValueDef elit@(TAST.ELit lit@(TAST.LInt 42 _) _) _) ->
                do
    ...
    --     case res of
    --       Left [UnifyError T.Float T.String s] -> shouldSpan s (1, 14) (1, 21)
    --       other                                -> unexpected other
    ...
    --       Left [UnifyError T.String T.Float s] -> shouldSpan s (4, 5) (4, 16)
    --       other                                -> unexpected other
    ...
    --             )
    --     case res of
    --       Left [UnifyError T.Float T.Char s] -> return () -- TODO: Fix -- shouldSpan s (4, 5) (4, 8)
    --       other                              -> unexpected other
    ...
    --             )
    --     case res of
    --       Left [UnifyError T.String T.Char s] -> return () -- TODO: Fix -- shouldSpan s (4, 10) (4, 13)
    --       other                               -> unexpected other
