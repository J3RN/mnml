  ...
  describe "Unification" $ do
    describe "literals" $ do
      it "unifies 'nUmeRiCs'" $ do
        ...
        res `shouldBe` Right (T.Var "num" (Set.singleton T.Numeric) 0)
    ...
    describe "lambdas" $ do
      it "unifies nullary lambdas" $ do
        ...
        res `shouldBe` Right (T.Fun [] (T.Var "num" (Set.singleton T.Numeric) 0))

      it "unifies identity" $ do
        let (res, _cs) = unify' "main = (x) => { x }"
        res `shouldBe` Right (T.Fun [T.Var "x" Set.empty 0] (T.Var "x" Set.empty 0))
      ...
      it "infers parameter types" $ do
        ...
        res
          `shouldBe` Right
            ( T.Fun
                [T.Var "num" (Set.singleton T.Numeric) 1]
                (T.Var "num" (Set.singleton T.Numeric) 1)
            )
      ...
      it "unifies value reference" $ do
        ...
        res `shouldBe` Right (T.Fun [] (T.Var "num" (Set.singleton T.Numeric) 2))

    describe "lists" $ do
      it "unifies empty list" $ do
        let (res, _cs) = unify' "main = []"
        res `shouldBe` Right (T.List (T.Var "elem" Set.empty 0))

      it "unifies singleton list" $ do
        let (res, _cs) = unify' "main = [1]"
        res `shouldBe` Right (T.List (T.Var "num" (Set.singleton T.Numeric) 0))
    ...
    describe "records" $ do
      ...
      it "unifies multiple field records" $ do
        ...
        res
          `shouldBe` Right
            (T.Record (Map.fromList [("foo", T.String), ("bar", T.Float), ("baz", T.Char)]))
    ...
    describe "application" $ do
        ...
        res
          `shouldBe` Right
            ( T.Fun
                ...
                ( T.Record
                    ( Map.fromList
                        [("float", T.Float), ("numeric", T.Var "num" (Set.singleton T.Numeric) 3)]
                    )
                )
            )
      ...
      it "allows functions with partial records to stay generic" $ do
        ...
        res
          `shouldBe` Right
            (T.Fun [] (T.Record (Map.fromList [("string", T.String), ("float", T.Float)])))

      it "unifies trivial circular reference" $ do
        let (res, _cs) = unify' (Text.unlines ["main = () => { foo() }", "foo = () => { main() }"])
        res `shouldBe` Right (T.Fun [] (T.Var "ret" Set.empty 1))
      ...
      it "unifies practical circular reference" $ do
        ...
        res
          `shouldBe` Right
            (T.Fun [T.Var "num" (Set.singleton T.Numeric) 1] (T.AlgebraicType "Bool"))
