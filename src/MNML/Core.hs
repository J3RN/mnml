...
module MNML.Core (Expr (..), Function (..), Modu (..), Memory (..), Type (..)) where

import Data.Text (Text)

newtype Funcref = Funcref Text
data Expr = Expr { _callee :: Text
                 , _args :: [Either Text Expr]
                 }

-- Ignoring vector types for the time being
data Type = I32 | F64 | FR Funcref

data Function = Function { _params :: [Type]
                         , _locals :: [Type]
                         , _return :: Type
                         , _body :: [Expr]
                         }

newtype Memory = Memory (Maybe Int)

data Modu = Modu { memories :: [Memory]
                 , functions :: [Function]
                 }
